...
def main():
    ...
    if(tempo_excedido == 0):
        ...
        if (time_passed >= max_tempo):
            ...
            receive_counts = sendcounts // num_disc
            receive_displacements = displacements // num_disc

    if(my_rank == 0):
        array_cr = np.empty(num_alunos, dtype=np.float64)
    else:
        array_cr = None
    ...
    if(tempo_excedido == 0):
        ...
        if (time_passed >= max_tempo):
            ...
            comm.Gatherv(local_array_cr,[array_cr, receive_counts, receive_displacements, MPI.DOUBLE], root=0)
